...
Seems like we can't include a bibliography element which is a shame!
...
Let's try this out then, using GIS code from last week. Inserting a chunk below:
```{r 1st choropleth map, echo=TRUE}
library(tidyverse)
library(geojsonio)
library(sf)
library(tmap)
...
```
Zotero bibliography helpful stuff is below

```{r package citr install}
#install.packages("citr")
library (citr)
```
Lets try adding a citation:
I got my reference from here Katie
So the citations don't seem to work which is annoying...we'll have to come back to that[@noauthor_hello_nodate]. 
